...
function SuperAdminCurrentMonth() {
    ...
    useEffect(() => {
        setLoading(true)
        postResource('SuperAdmin','MontlyIncomes', token,  null, {year: date, month: ownerClinics?.month_key}).then((response) => {
            setData(response)
            setLoading(false)
        });
...
export default SuperAdminCurrentMonth;