...
import ClinicRadiologyTasks from '../Radiology/Fragments/ClinicRadiologyTasks'


const resource = "Clinic";

function RadiologyHomeVisit({tab}) {
    ...
    let type = "radiology_home_visit";

    useEffect(() => {
        setLoading(true)
        postResource(resource,'WorkingHours',token,params.id,{service:'radiology_home_visit'}).then(responses => {
            setData(responses)
            setLoading(false)
        })
    ...
    const onFinish = (values,prevValues) => {
        ...
        if(values.sync_with_main) {
            values.service = 'radiology_home_visit'
...
export default RadiologyHomeVisit;