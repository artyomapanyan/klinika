...
function PhysicalTherapyHomeVisit({tab}) {
    ...
    let type = "physical_therapy_home_visit";

    useEffect(() => {
        setLoading(true)
        postResource(resource,'WorkingHours',token,params.id,{service:'physical_therapy_home_visit'}).then(responses => {
            setData(responses)
            setLoading(false)
        })
    ...
    const onFinish = (values,prevValues) => {
        ...
        if(values.sync_with_main) {
            values.service = "physical_therapy_home_visit"
...
export default PhysicalTherapyHomeVisit;